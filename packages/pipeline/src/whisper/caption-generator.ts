...
export async function generateCaptions(
  ...
  // Descriptions are now generated by the vision module (GPT-4o/Claude)
  // in the pipeline orchestrator using the video file directly.
  // See packages/pipeline/src/vision/ for the implementation.
...
async function runWhisperCpp(
  ...
  // Word-level timestamps: force max segment length to 1 character
  if (config.wordTimestamps) {
    args.push('-ml', '1')
    console.log('[WhisperCpp] Word-level timestamps enabled')
  }
...
async function runPythonWhisper(
  ...
  // Build command
  const wordTimestamps = config.wordTimestamps ? 'True' : 'False'
  const script = `import whisper; model = whisper.load_model("${modelName}"); ` +
    `result = model.transcribe("${wavPath}", language=${config.language === 'auto' ? 'None' : `"${config.language}"`}, word_timestamps=${wordTimestamps}, verbose=${config.verbose ? 'True' : 'False'}); ` +
...
/**
 * Generate the captions VTT file with optional word-level timestamps.
 * When wordTimestamps is enabled and words data is available, generates
 * per-word cues for maximum precision.
 */
async function generateCaptionsVtt(
  ...
  // If whisper.cpp already generated a VTT, check if it's our 1st pass output
  ...
  // Use word-level VTT if enabled and segments have word data
  const hasWordData = transcription.segments.some(
    s => s.words && s.words.length > 0
  )

  if (config.wordTimestamps && hasWordData) {
    console.log('[CaptionGenerator] Generating word-level captions VTT')
    await writeVttWordLevel(transcription.segments, outputPath, 'captions')
  } else {
    if (config.wordTimestamps) {
      console.log('[CaptionGenerator] Word timestamps requested but no word data available, using segment-level')
    }
...
/**
 * Write a VTT file with word-level timestamp precision.
 * Each word gets its own cue line with exact start/end times.
 * Words are grouped into short phrases (~5-7 words) for readability
 * while keeping per-word timing precision.
 */
async function writeVttWordLevel(
  segments: TranscriptionSegment[],
  outputPath: string,
  ...
  // Collect all words with their timestamps from all segments
  ...
  for (const seg of segments) {
    if (seg.words && seg.words.length > 0) {
      ...
      // Fall back: split segment text into pseudo-words with proportional timing
      const text = seg.text.trim()
      if (!text) continue
      const pseudoWords = text.split(/\s+/)
      const duration = seg.end - seg.start
      const wordDuration = duration / pseudoWords.length

      for (let i = 0; i < pseudoWords.length; i++) {
        const word = pseudoWords[i]!
        allWords.push({
          word,
  ...
  if (allWords.length === 0) {
    ...
    await writeVttFromSegments(segments, outputPath, kind)
    return
  }

  // Group words into short phrases for readability (target: 5-7 words per cue)
  // but each word keeps its precise timing boundary
  const WORDS_PER_CUE = 6

  for (let i = 0; i < allWords.length; i += WORDS_PER_CUE) {
    const group = allWords.slice(i, i + WORDS_PER_CUE)
    if (group.length === 0) continue

    const cueStart = group[0]!.start
    const cueEnd = group[group.length - 1]!.end

    // Ensure minimum duration for readability (200ms)
    const minDuration = 0.2
    const adjustedEnd = Math.max(cueEnd, cueStart + minDuration)

    const startStr = formatVttTime(cueStart)
    const endStr = formatVttTime(adjustedEnd)

    // Build the cue line with all words (VTT doesn't use HTML entities)
